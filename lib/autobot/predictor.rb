  ...
  class Predictor
    ...
    def predict_answer_to(question)
      ...
      valid_answers = model_data
        .sort_by { |data| WordVec.new(data.first).similarity_to(input_vec) }
        .slice(-3..-1)
    ...
    def extract_morphemes_from(sentence)
      Sentence.new(sentence).morphemes.select do |morpheme|
    ...
    def vectorize(sentence)
      ...
      WordVec.from_words(words: words, idf_table: idf_table)
    end
  end
end
